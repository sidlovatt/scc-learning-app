...
function WatchStage({ onComplete }) {
  ...
  return (
    <DndProvider onDragEnd={handleDragEnd}>
      <div className="ck10am-stage">
        <p className="ck10am-hint">Round 1: Drag each watch name onto its time period.</p>
        <div className="ck10am-zones">
          {watches.map((w) => (
            <Droppable key={w.id} id={w.id} className="ck10am-zone">
              <span className="ck10am-zone-label">{w.range}</span>
              {matched[w.id] && <Card status="locked">{w.name}</Card>}
            </Droppable>
          ))}
        </div>
        <div className="ck10am-pool">
          {remaining.map((w) => (
        ...
        <p className="ck10am-progress">{Object.keys(matched).length} / {total} matched</p>
...
function BellsStage({ onComplete }) {
  ...
  return (
    <DndProvider onDragEnd={handleDragEnd}>
      <div className="ck10am-stage">
        <p className="ck10am-hint">Round 2: Drag each time onto the number of bells struck.</p>
        <div className="ck10am-bell-zones">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
            <Droppable key={n} id={String(n)} className="ck10am-bell-zone">
              <span className="ck10am-zone-label">{n} bell{n > 1 ? 's' : ''}</span>
              <div className="ck10am-bell-zone-items">
...
function PatternStage({ onComplete }) {
  ...
  return (
    <DndProvider onDragEnd={handleDragEnd}>
      <div className="ck10am-stage">
        <p className="ck10am-hint">Round 3: Drag each strike pattern onto its number of bells.</p>
        <div className="ck10am-bell-zones">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
            <Droppable key={n} id={String(n)} className="ck10am-bell-zone">
              <span className="ck10am-zone-label">{n} bell{n > 1 ? 's' : ''}</span>
...
export default function CK10AMatch({ onBack }) {
  const [stage, setStage] = useState('watches');

  return (
    <div className="ck10am">
      <Button variant="ghost" onClick={onBack} className="ck10am-menu-back">
        Back to Menu
      </Button>
